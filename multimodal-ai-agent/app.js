...
class Node {
  ...
  async processTextToImage(input) {
    ...
    try {
      ...
      // Call GPT Image API to generate an image
      ...
      if (!response.ok) {
        ...
        throw new Error(`Image generation API request failed: ${errorData.error?.message || 'Unknown error'}`);
      }
  ...
  async processImageToText(input) {
    ...
    try {
      ...
      // Call GPT-4o API to analyze the image
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(`Image analysis API request failed: ${errorData.error?.message || 'Unknown error'}`);
      }
