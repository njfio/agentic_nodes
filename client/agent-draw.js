...
(function() {
  ...
  // Add a method to handle double-clicks on agent nodes
  window.AgentNodes.handleAgentNodeDoubleClick = function(node) {
    console.log(`Double-clicked on agent node ${node.id}`);

    // Check if the openAgentNodeEditor method exists
    if (typeof window.AgentNodes.openAgentNodeEditor === 'function') {
      console.log(`Opening agent node editor for node ${node.id}`);
      window.AgentNodes.openAgentNodeEditor(node);
    } else {
      console.error('openAgentNodeEditor method not found in AgentNodes');

      // Try to find the agent node editor modal
      ...
      if (!agentNodeEditor) {
        ...
        if (window.App && typeof window.App.openNodeEditor === 'function') {
          console.log(`Falling back to regular node editor for node ${node.id}`);
          window.App.openNodeEditor(node);
        }
        return;
      }

      // If the modal exists but the method doesn't, create a simple implementation
      console.log(`Creating fallback openAgentNodeEditor method for node ${node.id}`);
